...
public class VideoScreen extends Screen {
	...
	private DecodeAndPlayVideo videoDecoder;

	public VideoScreen(Application app, String filename) {
		super(app);
		this.filename = filename;
		this.frames = new ArrayList<BufferedImage>();
		this.videoDecoder = new DecodeAndPlayVideo();
	...
	public void onDraw(Graphics2D g) {
		if (!frames.isEmpty()) {
			BufferedImage frame = frames.remove(0);
	...
	private class DecodeAndPlayVideo implements Runnable {
		...
		@Override
		public void run() {
			decodeAndPlayVideo(VideoScreen.this.filename);
		}

		public void decodeAndPlayVideo(String filename) {
			...
			while (container.readNextPacket(packet) >= 0) {
				...
				if (packet.getStreamIndex() == videoStreamId) {
					...
					if (picture.isComplete()) {
						...
						VideoScreen.this.frames.add(Utils.videoPictureToImage(newPic));
