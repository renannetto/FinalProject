...
public class FinalMap extends Graph<FinalNode> {
	...
	public static Vec2i toTileCoordinates(Vec2f position) {
		int gridX = (int) Math.floor(position.x / tileDimensions.x);
		int gridY = (int) Math.floor(position.y / tileDimensions.y);
		return new Vec2i(gridX, gridY);
	}

	public static Vec2f toWorldCoordinates(Vec2i tilePosition) {
		return new Vec2f((tilePosition.x * tileDimensions.x) + tileDimensions.x
				/ 2, (tilePosition.y * tileDimensions.y) + tileDimensions.y / 2);
	}
